...
const char* filenames[textureCount] = { "water.jpg", "animal.jpg" };
const char* shader_textureName[textureCount] = { "u_map1", "u_map2" };
GLuint texID[textureCount];
GLint g_mapLocation[textureCount];
...
bool createShaderProgram() {
    ...
    const GLchar vsh[] =
        ...
        //"float f(vec2 p) {return 8 * atan(p.x * p.y);}"
        //"vec3 grad(vec2 p) {return vec3(-8 * p.y / (p.x * p.x * p.y * p.y + 1), 1.0, -8 * p.x / (p.x * p.x * p.y * p.y + 1));}"
        "float f(vec2 p) {return 0.04 * (1 - p.x * p.y) * sin(1 - p.x * p.y);}"
        "vec3 grad(vec2 p) {return vec3(0.04 * p.y *(sin(1 - p.x * p.y) +  (1 - p.x * p.y) * cos(1 - p.x * p.y)), 1.0, 0.04 * p.x * (sin(1 - p.x * p.y) + (1 - p.x * p.y) * cos(1 - p.x * p.y)));}"
        ""
        "void main()"
        "{"
        "    float y = f(a_pos);"
    ...
    "void main()"
    "{"
    "    o_color = texture(u_map, v_texCoord);"
    "}"*/
    ...
    const GLchar fsh[] =
        ...
        "   vec3 color = vec3(mix(sin(tex1), cos(tex2), 0.5));"
        ""
        "o_color = vec4(color * d + s * vec3(1.0, 1.0, 1.0), 1.0); "
        //"   o_color = vec4(color * d + s * vec4(1.0));"
        "}"
        ;
...
bool loadTexture(const char* filenames[])
{
    ...
    for (int i = 0; i < textureCount; i++)
    {
        ...
        GLvoid* image = stbi_load(filenames[i], &texW, &texH, &nrChannels, 0);
        ...
        g_mapLocation[i] = glGetUniformLocation(g_shaderProgram, shader_textureName[i]);
        
    
    }
    return g_mapLocation != 0;
...
bool createModel() {
    ...
    for (int z = 0; z < n; z++)
        for (int x = 0; x < n; x++)
        {
            vertices[k++] = (t0 + x) * 0.1f;
            vertices[k++] = (t0 + z) * 0.1f;
...
bool init() {
    ...
    return createShaderProgram() && createModel() && loadTexture(filenames);
}

void reshape(GLFWwindow* window, int width, int height) {
    P = perspective(radians(45.0f),float(width) / float(height), 0.1f, 100.f);
    glViewport(0, 0, width, height);
}
...
void draw(double delta)
{
    ...
    for (int texUnit = 0; texUnit < textureCount; texUnit++)
    {
        glActiveTexture(GL_TEXTURE0 + texUnit);
        glBindTexture(GL_TEXTURE_2D, texID[texUnit]);
